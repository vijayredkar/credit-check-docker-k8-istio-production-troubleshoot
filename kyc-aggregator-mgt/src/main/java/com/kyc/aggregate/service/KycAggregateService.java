...
public class KycAggregateService {
	...
	public KycResponse processKyc(String type, String headerValue, String creditCheck, String mongoFirstName) throws KycAggregateException  
	{
	    ...
	    */
		...
		{	
		    if(Constants.CREDIT_CHECK_TYPE_BASIC_REPORT.equals(type))//request routing
		    {
		    	...
		    	kycResponse = invokeService(crdChkBasicIstioUrl);
		    }
		    ...
		    else if(Constants.CREDIT_CHECK_TYPE_ADVANCED_REPORT.equals(type))//request routing
		    {
		    	...
		    	kycResponse = invokeService(crdChkAdvancedIstioUrl);	
		    }
		    ...
		    {	
		    	...
		    	//				 http://192.168.49.2:31291/credit-check/advanced
			    	...
			    	kycResponse = invokeServiceHdrRedirect(url, headerValue, mongoFirstName);
			    	System.out.println("##### vijay kycResponse "+kycResponse);
			    }
			    catch(Exception e)
			    {//works - Istio invocation - preferred
		    ...
		    else
		    {
		    	...
		    	kycResponse = invokeService(crdChkReportIstioUrl);
		    }
		    /*else
		    {
	...
	private KycResponse invokeService(String url) 
	{
	...
	private KycResponse invokeServiceHdrRedirect(String url, String headerValue, String mongoFirstName)
	{
		RestTemplate restTemplate = new RestTemplate();	
		KycResponse kycResponse;
		ResponseEntity<KycResponse> response;
		...
		//headers.set("kafka-topic-name", kafkaTopicName);
		headers.set("mongo-first-name", mongoFirstName);
